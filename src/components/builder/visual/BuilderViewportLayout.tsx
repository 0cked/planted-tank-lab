...
export function BuilderViewportLayout(props: BuilderViewportLayoutProps) {
  return (
    <div className="relative h-dvh w-full overflow-hidden bg-[#060d16]">
      ...
      {/* Left icon rail */}
      <nav className="pointer-events-auto absolute left-3 top-3 bottom-3 z-10 flex w-11 flex-col items-center gap-1 rounded-2xl border border-white/10 bg-black/50 py-2 backdrop-blur-xl">
        {props.iconRail}
      </nav>

      {/* Floating contextual panel (appears next to icon rail) */}
      {props.floatingPanel ? (
        <aside className="pointer-events-auto absolute left-[68px] top-3 bottom-3 z-10 w-[260px] overflow-auto rounded-2xl border border-white/10 bg-black/55 p-3 shadow-2xl backdrop-blur-xl">
          {props.floatingPanel}
        </aside>
      ) : null}
