    ...
    <div className="space-y-3">
      ...
      {props.currentStep === "tank" ? (
        <div className="space-y-2">
          ...
          <section className="space-y-2 rounded-2xl border border-white/20 bg-slate-900/50 p-2.5">
            <div>
              <h3 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-200">
                Starter templates
              </h3>
              <p className="mt-1 text-[11px] text-slate-300">
                Start from curated layouts and customize from there.
              </p>
            </div>

            <div className="space-y-2">
              {props.templates.map((template) => (
                <article key={template.id} className="rounded-xl border border-white/15 bg-slate-950/55 p-2.5">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-xs font-semibold text-slate-100">{template.name}</div>
                      <p className="mt-1 text-[11px] leading-relaxed text-slate-300">{template.description}</p>
                      <div className="mt-1.5 flex flex-wrap gap-1">
                        {template.highlights.map((highlight) => (
                          <span
                            key={highlight}
                            className="rounded-full border border-white/15 bg-slate-900/70 px-1.5 py-0.5 text-[10px] text-slate-300"
                          >
                            {highlight}
                          </span>
                        ))}
                    ...
                    <button
                      ...
                      className={`inline-flex min-h-11 min-w-11 touch-manipulation items-center justify-center rounded-md border px-3 py-2 text-[10px] font-semibold ${
                        template.available
                          ...
                          : "cursor-not-allowed border-white/15 bg-slate-800/55 text-slate-500"
                      }`}
                    >
                      Use template
                    </button>
                  ...
                  {!template.available && template.unavailableReason ? (
                    <p className="mt-1.5 text-[10px] text-amber-200/85">{template.unavailableReason}</p>
                  ) : null}
                </article>
              ))}
            </div>
