...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!supabaseUrl || !serviceRole) {
    const u = new URL(`/admin/products/${id}`, req.url);
  ...
  const productRows = await db
    .select({ slug: products.slug, imageUrls: products.imageUrls })
    .from(products)
  ...
  const key = `products/${product.slug}/${Date.now()}-${nanoid(10)}.${ext}`;
  ...
  const prev = Array.isArray(product.imageUrls)
    ? product.imageUrls.filter((x): x is string => typeof x === "string" && x.length > 0)
    : [];
